...
const typescriptSlice = createSlice({
  name: "typescript",
...
const questionSelector = (index: number) => {
  return (state: IState) => {
    return state.typescript.questions[index];
  };
};

const allQuestionSelector = (state: IState) => {
  return state.typescript.questions;
};

const questionLengthSelector = (state: IState) => {
  return state.typescript.totalQuestions;
};
...
} = typescriptSlice.actions;

export default typescriptSlice.reducer;
